...
impl SignablePacket {
    pub fn signing_bytes(&self) -> Result<Vec<u8>> {
        ...
        let payload =
            postcard::to_allocvec(self).map_err(|e| AgentZkError::Serialization(e.to_string()))?;
...
impl PckpPacket {
    pub fn content_hash(&self) -> B3 {
        let bytes = postcard::to_allocvec(self).expect("PCKP packet serializes for content hash");
        combine("agentzk.pckp.packet.v0", &[&bytes])
    }
